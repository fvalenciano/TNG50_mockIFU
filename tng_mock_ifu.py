...
def TNG_data_loader(subhalo_ID, snapshot, radius, v_f, Stars):
    ...
    if Stars == True:
        ...
        h5filename = (f'TNGdata_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}')
        ...
        h5datafolder = base + 'TNGdata_' + str(subhalo_ID) + '/'
        os.makedirs(h5datafolder, exist_ok=True)
        h5filename = (f'All_TNGdata_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}')
        #filename = filename.replace('.', 'c')
        h5filename = h5filename + '.hdf5'
        h5filepath = h5datafolder + h5filename
...
def read_hdf5file(subhalo_ID, snapshot, radius, v_f, Stars):
    ...
    '''
    if Stars == True:
        filepath = base + (f'TNGdata_{subhalo_ID}/TNGdata_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}.hdf5')
        ...
        filepath = base + (f'TNGdata_{subhalo_ID}/All_TNGdata_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}.hdf5')
...
def Galaxy_ColorHistogram(subhalo_ID, snapshot, radius, v_f, num_bins, Stars=True):
    ...
    if Stars == True:
        ...
        for prop in props:  
            npyfilename = (f'TNGhist_{prop}_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}bins.npy')
...
def mk_hist_plots(subhalo_ID, snapshot, radius, v_f, Stars, vmin=None, vmax=None, *, num_bins=None, binsize=None):
    ...
    if Stars == True:
        ...
        def load_hist_npy(prop, snapshot, radius, v_f):
            ...
            filename = (f'TNGhist_{prop}_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}bins')
            filepath = datafolder + filename + '.npy'
            return np.load(filepath)

        datafolder = (base + f'TNGdata_{subhalo_ID}/Histograms/')
        filename = (f'TNGhist_Metallicity_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}bins')
        filepath = datafolder + filename + '.npy'
        ...
        os.makedirs(datafolder, exist_ok=True)
        filename = (f'TNGhist_Stars_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}')
        ...
        filepath = (f'TNGdata_{subhalo_ID}/All_TNGdata_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}.hdf5')
        TNG_data = read_hdf5file(subhalo_ID, snapshot, radius, v_f, Stars)
        ...
        datafolder = base + (f'TNGdata_{subhalo_ID}/Histograms/Plots/')
        os.makedirs(datafolder, exist_ok=True)
        filename = (f'TNGhist_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}bins')
        filepath = datafolder + filename + '.png'
        plt.savefig(filepath)
        plt.show()
...
def SyntheticSpectra(TNG_data, subhalo_ID, snapshot, radius, v_f, method, grid, flux, output_name, save_plots):
    ...
    if method == 'Nearest':
        ...
        if save_plots == True:
            ...
            os.makedirs(datafolder, exist_ok=True)
            filename = (f'SS_{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{method}.npy')
            filepath = datafolder + filename  
...
def DataCube(subhalo_ID, snapshot, radius, v_f, method='Interpolation', i_ls = False, *, binsize=None, num_bins=None):
    ''' Creates the IFUs:
    ---------------------
    Input:
    ...
    filename = (f'DCresults_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{method}_{num_bins}.hdf5')
    ...
    dcfilename = (f'DC_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}.fits')
    ...
    dcfilename_mag = (f'MI_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}.fits')
...
def superficial_magnitude(spectrum, wavelength, binsize, filterpath):
    ...
    obs_spectrum = spectrum# * (wavelength**2) / c_AAs
...
def pass_to_jwst(subhalo_ID, snapshot, radius, v_f, method = 'Interpolation', *, binsize=None, num_bins=None):
    ...
    if snapshot == 99:
        ...
        dataname = (f'DC_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}_z1.50.fits')
        ...
        os.makedirs(savefolder, exist_ok=True)
        filename_jwst = (f'JWST_DCresults_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}_z1.50.hdf5')
        ...
        dataname = (f'DCresults_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{method}_{num_bins}.hdf5')
        datapath = datafolder + dataname
        ...
        savefolder = base + (f'TNGdata_{subhalo_ID}/DC_SaveResults/JWST/')
        os.makedirs(savefolder, exist_ok=True)
        filename_jwst = (f'JWST_DCresults_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}.hdf5')
        filepath_jwst = savefolder + filename_jwst
        savepath_jwst = create_new_savefile(filepath_jwst, snapshot, num_bins, v_f, radius, method, M = len(data[:,0,0]))
    ...
    os.makedirs(dcfolder, exist_ok=True)
    dcfilename = (f'JWST_DC_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}.fits')
    dcfilepath = dcfolder + dcfilename
    ...
    dcfilename_mag = (f'JWST_MI_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{method}_{num_bins}.fits')
    dcfilepath_mag = dcfolder + dcfilename_mag
...
def displace_galaxy(subhalo_ID, snapshot, radius, v_f, sampling, *, num_bins=None, binsize=None):
    ...
    dcfilename_out = (f'DC_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}_z{myzout:4.2f}.fits')
    dcfilepath_out = dcfolder_out + dcfilename_out

    magfilename_out = (f'MI_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}_z{myzout:4.2f}.fits')
    ...
    # Replace negative vals by sth extremely small to avoid NaN in the log interp
    # mydata = np.maximum(mydata, 1.E-30)
    
    # This is the scaling factor to have the output also in units of flux / spa
    escalado = (mydata.shape[1]*mydata.shape[2]) / (float(ypix*xpix))
    # print("Escalado:" , '{:06.3f}'.format(escalado))
    ...
    #temp1_mag = np.log10(mydata_mag[:,:])
    #temp2_mag = resize(temp1_mag, (ypix, xpix))
    #temp3_mag = 10.**(temp2_mag)
    mydata2_mag = resize(data_mag, (ypix, xpix))

    print("Fin rebineado")
    # numberofnan = np.count_nonzero(np.isnan(mydata2))
...
def plotting_fluxes(subhalo_ID, snapshot, radius, v_f, method='Interpolation', i_ls = False, normalization=False, jwst=False, harmoni=False, *, binsize=None, num_bins=None):
    ...
    if harmoni == False:
        ...
        ax_main.text(0.02, 0.85, (f'Radius: {radius} kpc'), fontsize = 17, color='white', transform=ax_main.transAxes, verticalalignment='center', horizontalalignment='left')    
        ...
        if jwst== True:
            plt.savefig(f'DC_JWST_{subhalo_ID}_{snapshot}_{radius}.png')
    
        else:
            plt.savefig(f'DC_{subhalo_ID}__{snapshot}_{radius}.png')
        ...
        dcfilename = (f'DC_snap0{snapshot}_{radius}kpc_{v_f[0]:0.1f}_{v_f[1]:0.1f}_{v_f[2]:0.1f}_{num_bins}_noiseless_obj.fits')
        dcfilepath = dcfolder + dcfilename
